...
class EditorPrompt(QDialog):
    def __init__(self=None, pipeline=None, on_run_cmd=None):
        super().__init__()
        ...
        self.open_files_section = OpenFilesSection(self, self.pipeline, self.update)
        self.open_files_section.setAlignment(Qt.AlignTop)
        self.layout.addLayout(self.open_files_section)

        self.process_logs_section = ProcessLogsSection(self, self.pipeline, self.update)
        ...
        self.color_logs_section = ColorLogsSection(self, self.pipeline, self.update)
        ...
        self.filter_logs_section = FilterLogsSection(self, self.pipeline, self.update)
        self.filter_logs_section.setAlignment(Qt.AlignTop)
        hbox.addLayout(self.filter_logs_section)

        self.layout.addLayout(hbox)

        self.preview_logs_section = PreviewLogsSection(self, self.pipeline)
